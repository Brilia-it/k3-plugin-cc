...
async function assertSwarmSupported(context: CommandContext, minMinor: number): Promise<void> {
  ...
  if (probe.kind !== "ok") {
    if (minMinor >= WRITE_SWARM_MIN_MINOR) {
      throw new RuntimeError(
        "SWARM_UNSUPPORTED",
        `/k3:swarm --write needs a CONFIRMED kimi-code >= 0.${WRITE_SWARM_MIN_MINOR}.0 (the hard AgentSwarm concurrency cap required for safe concurrent writes), but the version probe failed: ${probe.reason}. Ensure kimi-code is on PATH and retry.`,
        "swarm.version-gate",
      );
    }
    return;
