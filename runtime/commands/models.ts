...
// A projection, not an upstream config validator or an authentication probe.
// Never serialize parsed config, parser errors, provider entries, or env bags.
...
export async function runModelInventory(argv: string[], context: CommandContext): Promise<string> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") configPresent = false;
  ...
  try {
    ...
    const models: ModelEntry[] = Object.entries(record(config.models)).map(([alias, value]) => {
      const entry = normalized(value);
      return {
        alias,
    ...
    if (argv[0] === "--json") return JSON.stringify(inventory, null, 2);
    const quoted = (value: string | null): string => value === null ? "(not configured)" : JSON.stringify(value);
    return [
...
function inventoryError(): RuntimeError {
...
function normalized(value: unknown): Record<string, unknown> {
  ...
  for (const [key, item] of Object.entries(record(value))) {
    ...
    if (Object.hasOwn(result, normalizedKey)) throw new Error();
