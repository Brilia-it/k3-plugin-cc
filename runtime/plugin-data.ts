...
export function resolvePluginDataRoot(
  ...
  const shared = [env.CLAUDE_PLUGIN_DATA, env.PLUGIN_DATA].filter(
...
function installedDataRoot(packageRoot: string): string | undefined {
...
function sameDataRoot(left: string, right: string): boolean {
...
function canonicalDataRoot(value: string): string | undefined {
  let cursor = path.resolve(value);
  const missing: string[] = [];
  for (;;) {
    try {
      const resolved = realpathSync(cursor);
      if (missing.length > 0 && !statSync(cursor).isDirectory()) return undefined;
