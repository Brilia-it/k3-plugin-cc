...
- **you hold the `job_id` — the user should never have to type it.** It is a raw UUID. Keep it from the launch output and act on plain language: cancel with `${CLAUDE_PLUGIN_ROOT}/scripts/companion.sh cancel <job-id>` when the user says to stop (a graceful cancel — SIGTERM-based teardown is a settlement barrier, so partial work is written out), and check `${CLAUDE_PLUGIN_ROOT}/scripts/companion.sh status <job-id>` when they ask how it is going
