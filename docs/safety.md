...
- **No clobbering.** `/k3:setup` (Claude) and `$k3-setup` (Codex) each write
...
- **Migration + cleanup.** A pre-v1.7.0 un-suffixed block is adopted and
  converted in place by the current host on its next `/k3:setup`. Orphaned,
  ...
  only tables whose command path derives to the *current* host. `/k3:setup
  --uninstall` removes only the current host's block (plus legacy + this host's
  orphans); `/k3:setup --uninstall --all` removes every host's block and every
...
- **Markers are not durable — verification does not depend on them (v1.8.2).**
  ...
  applies over duplicate/orphan/invalid marked states. The next `/k3:setup`
...
For `/k3:rescue`, the hook delegates to [`evaluateRescueHookRequest`](../runtime/rescue-approval.ts). This is the same workspace-bound allowlist code that v0.4 carried — kept verbatim from PR 3's port. Highlights:
...
`/k3:rescue` intentionally allows test/build runners (`go test`, `cargo test`, `pytest`, `python -m pytest`, `<pm> test`). **Running tests runs the workspace's own code** — test functions, `build.rs`, `conftest.py`, fixtures. The allowlist stops these from writing *outside* the workspace (their report-writing flags are rejected, above), but it cannot and does not sandbox the code they execute. This is the same trust the developer already extends to their own repo. If you run `/k3:rescue` in a workspace you do not trust, assume any allowed test runner can execute arbitrary code from that workspace.
...
- **Model-invocable agent (v1.5), patch-only by construction.** Following the field-proof above, `--write` now has a `k3-swarm-write` subagent so the main Claude thread can dispatch a write fan-out on its own judgement — the same human-command + model-invocable-agent split the rest of the suite uses. Auto-dispatch is a safe extension here because it widens **no** write surface and removes **no** bound: the agent path runs the identical `runSwarm --write` code, so edits still land only in the throwaway worktree, the managed hook still gates every `coder` subagent, the patch is still handed back for the user to apply (**the plugin never applies or commits**), and `--budget` + default `--max-concurrency` 1 + the hook requirement all still hold. In fact write-swarm is a *less* dangerous agent than the `k3-pursue` agent already shipped: pursue writes directly into the workspace, whereas write-swarm's output is a patch the human reviews before anything reaches a real tree. Because it is still a write surface, the agent description demands BOTH signals — many disjoint write targets AND explicit fan-out intent — and steers single edits to `k3-rescue`, read-only fan-outs to `k3-swarm`, and autonomous loops to `k3-pursue`. Default `--max-concurrency` stays **1**; raising it (concurrent writers) remains a deliberate, human-or-explicitly-requested choice.

## The setup probe

`/k3:setup` runs a two-layer probe before reporting success:
...
`/k3:setup --check` is strict and exits nonzero on failure. It rejects an unloadable TOML file, any invalid configured hook, and any managed block that doesn't pass the [`parseManagedBlock`](../runtime/hooks/managed-block.ts) grammar (missing `[[hooks]]` table, wrong event, present `matcher = ...` line, duplicate blocks, orphan markers, or a stale hook-script path). `/k3:setup` (without `--check`) can replace a malformed managed block, but it will not overwrite or silently work around invalid foreign config: if the complete candidate file would not load, setup leaves the existing file unchanged and identifies the bad hook or TOML line.
...
The block should match the template at the top of this document. If the `[[hooks]]` line is missing, if `event` is something other than `"PreToolUse"`, or if there's a `matcher = ...` line — the installer should have rejected the block. Run `/k3:setup --uninstall` + `/k3:setup` to repair.
...
The plugin's agents read that field: on `hook-script` (`retryable_after_setup: true`) they run `/k3:setup` for that host and retry **once**. On `node-bin`/`both` they surface the refusal and stop — a moved interpreter may be *gone*, which is a real enforcement gap rather than a moved file, and blind re-pinning would paper over it.
...
If you find a way to coax /k3:review, /k3:challenge, /k3:review_gate, or /k3:ask into running a non-read tool — or to make /k3:rescue mutate `.git/` or escape the workspace — please open a GitHub security advisory rather than a public issue.
