...
`runtime/cli-client.ts` fails closed before process creation whenever `KIMI_CODE_EXPERIMENTAL_FLAG` has one of the truthy values `1`, `true`, `yes`, or `on` (trimmed and case-insensitive). The refusal is `CLI_V2_HOOK_ORDER_UNSAFE` with `refusal_kind:"v2-hook-order-unsafe"`. It is **not** hook-install drift and `/k3:setup` is not the remedy; unset the experimental flag.
