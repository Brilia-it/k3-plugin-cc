...
| Experimental-v2 veto-listener order | `packages/agent-core-v2/src/index.ts`, `agent/toolExecutor/beforeToolExecuteEvent.ts`, `agent/plan/planService.ts`, `agent/externalHooks/externalHooksService.ts`, `app/sessionLifecycle/sessionLifecycleService.ts` | External PreToolUse must run before **every final `event.allow()`**, not merely before tool execution. 0.31.0 proved why: plan service registers before external hooks and final-allows its exact plan-file write; `default_plan_mode=true` reaches it on a fresh session. Until an exact release fixes the order, the plugin must refuse upstream-equivalent truthy `KIMI_CODE_EXPERIMENTAL_FLAG` values before spawn. | `runtime/cli-client.ts` v2 refusal; `runtime/hooks/approval-policy.ts` denies `EnterPlanMode` as defense in depth |
