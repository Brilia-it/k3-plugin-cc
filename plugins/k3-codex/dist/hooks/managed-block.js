...
export function evaluateInstalled(contents, expectedCommand, opts) {
    ...
    if (state.kind === "absent") {
        ...
        if (hasCleanEnforcingHookEntry(contents, expectedCommand)) {
            return {
                installed: true,
                via: "bare-table",
                note: "this host's managed-block markers are missing (kimi-code rewrites its config on login/settings changes and strips all comments), but the hook table with the exact canonical command is present and enforcing. Run /k3:setup to re-adorn the markers.",
                state,
            };
        }
    ...
    if (!foundHookEntryIsClean(contents, expectedCommand)) {
        return {
            installed: false,
            reason: "the managed block's hook table carries a matcher or an unexpected key OUTSIDE the marked body (e.g. after the END marker) — kimi-code would load it and disable the hook. Run /k3:setup --uninstall, then /k3:setup to repair.",
            state,
        };
    }
